...
% fit_params = [-5.260627e+03 3.276345e+02 23.526061];
% fit_params = [-4.584696e+03 -2.396726e+02 41.655948 21.617160 1.235856];
% fit_params = [-4.583398e+03 -6.003066e+02 3.478569e+02 -75.211621 5.871371 21.792407 1.098147];
fit_params = [-4.504466e+03 -1.700262e+02 -2.260201e+02 72.781164 38.563085 -16.423412 1.550770 21.419482 1.274859];
...
ax1 = gca();
xlabel(ax1, "Pressure (Pa)", FontSize=26*scale);

ylabel(ax1, "CO_2 Uptake (mol/kg)", FontSize=26*scale)
legend(ax1, legend_str, FontSize=20*scale, Location="best", NumColumns=1);
...
ax1 = gca();
xlabel(ax1, "CO_2 Uptake (mol/kg)", FontSize=26*scale);

ylabel(ax1, "log Pressure (log Pa)", FontSize=26*scale)
% legend(ax1, legend_str, FontSize=20*scale, Location="best", NumColumns=1);
...
Q_ads = 8.314 .* polyval(flip(fit_params(1:num_a)), q_lin)./1e03;  % kJ/mol
...
ff = 5;
e1 = errorbar(data_1(1:end-ff, 1), abs(data_1(1:end-ff, 2)), data_1(1:end-ff, 3));
e2 = errorbar(data_2(:, 1), abs(data_2(:, 2)), data_2(:, 3));
e3 = errorbar(data_3(:, 1), abs(data_3(:, 2)), data_3(:, 3));
...
e1.Color = "#EDB120";
e1.LineWidth = 2.0*max_scale;
e1.DisplayName = "273K";

e2.CapSize = sz;
e2.MarkerFaceColor = "#D95319";
e2.Color = "#D95319";
e2.LineWidth = 2.0*max_scale;
e2.DisplayName = "298K";

e3.CapSize = sz;
e3.MarkerFaceColor = "#0072BD";
e3.Color = "#0072BD";
e3.LineWidth = 2.0*max_scale;
e3.DisplayName = "323K";
...
ylabel(ax1, strcat(char(8722), '\DeltaH_{ads} (kJ/mol)'), 'Interpreter', 'tex', FontSize=26*scale)
% legend(ax1, {'GCMC-273K', 'GCMC-298K','GCMC-323K', 'HeatFit'}, FontSize=20, Location="best", NumColumns=1);
% legend(ax1, {'GCMC (273K)', 'GCMC (298K)','GCMC (323K)', 'HeatFit'}, FontSize=20*scale, Location="best", NumColumns=1);
